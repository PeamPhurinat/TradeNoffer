...
async def get_all_posts(request: Request, db: Session = Depends(get_db)):
    try:
        ...
        # Find items that are involved in pending offers
        ...
        # Find items that the current user has already sent offers for
        items_user_made_offers_for = db.query(TradeOffer.receiver_item_id).filter(
            TradeOffer.sender_id == user_id
        ).subquery()
        items_user_made_offers_fora = db.query(TradeOffer.sender_item_id).filter(
            TradeOffer.receiver_id == user_id
        ).subquery()
        
        # Get all items except:
        # 1. Those owned by current user
        # 2. Those in pending offers
        # 3. Those the user has already made offers for
        items = db.query(Item).filter(
            Item.userID != user_id,
            ~Item.ID.in_(items_in_pending_offers),
            ~Item.ID.in_(items_user_made_offers_for),
            ~Item.ID.in_(items_user_made_offers_fora),
        ).all()
